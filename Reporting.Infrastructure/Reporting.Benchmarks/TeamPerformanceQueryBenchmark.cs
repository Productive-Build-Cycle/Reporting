...
public class TeamPerformanceQueryBenchmark
{
    ...
    // Additional benchmarks with different filter scenarios
    [Benchmark]
    [BenchmarkCategory("Filtered")]
    public List<TeamPerformanceSummaryResponseDto> EfCore_WithDateFilter()
    {
        var request = new TeamPerformanceSummaryRequestDto
        {
            StartDate = DateTime.UtcNow.AddMonths(-1),
            EndDate = DateTime.UtcNow
        };
        return _efQuery.ExecuteAsync(request).GetAwaiter().GetResult();
    }

    [Benchmark]
    [BenchmarkCategory("Filtered")]
    public List<TeamPerformanceSummaryResponseDto> Dapper_WithDateFilter()
    {
        var request = new TeamPerformanceSummaryRequestDto
        {
            StartDate = DateTime.UtcNow.AddMonths(-1),
        ...
        return _dapperQuery.ExecuteAsync(request).GetAwaiter().GetResult();
    }

    [Benchmark]
    [BenchmarkCategory("Filtered")]
    public List<TeamPerformanceSummaryResponseDto> StoredProcedure_WithDateFilter()
    {
        var request = new TeamPerformanceSummaryRequestDto
        {
            StartDate = DateTime.UtcNow.AddMonths(-1),
        ...
        return _storedProcedureQuery.ExecuteAsync(request).GetAwaiter().GetResult();
    }
}
